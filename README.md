...
`terraform/` provisions the full AWS deployment target: a VPC, one EC2 host running the app via Docker Compose, a managed Postgres database, and a CloudFront + S3 edge for the frontend. Below is what each AWS service is used for and which `.tf` file owns it.

- **VPC / Networking** (`network.tf`) — one VPC with a public subnet (the EC2 instance) and private subnets across multiple AZs (RDS only). The private route table has no route to the internet gateway and there's no NAT gateway — RDS needs no outbound internet access, and a NAT would be the single largest recurring cost.
- **EC2** (`ec2.tf`) — one Amazon Linux 2023 instance (size from `var.instance_type`) sitting behind an **Elastic IP** (`aws_eip`, pinned so a stop/start doesn't silently break the CloudFront origin). `user_data` does a minimal first-boot bootstrap only — installs Docker, Ansible, Tailscale, and the SSM agent, then installs a oneshot systemd unit that pulls the real convergence bundle from the artifacts S3 bucket and runs it via Ansible (`ansible/site.yml`). All actual host state (compose stack, nginx proxy, Tailscale join) is applied by that playbook, not by `user_data` itself.
- **Security Groups / IAM (instance)** (`security.tf`) — the instance security group allows port 80 only from CloudFront's AWS-managed IP prefix list (blocks bypassing the CDN) plus 443 from `var.http_ingress_cidr`; there is no SSH ingress at all, admin access goes through **SSM Session Manager**. The RDS security group only allows Postgres (5432) from the instance's security group. The EC2 instance role attaches `AmazonSSMManagedInstanceCore` and `AmazonEC2ContainerRegistryReadOnly`, plus an inline policy for `kms:Decrypt` (SSM's managed key) and read-only access to its own `secret/*` and `config/*` SSM parameters and `s3:GetObject` on the artifacts bucket.
- **ECR** (`ecr.tf`) — one repository per image (`backend`, `frontend`), scan-on-push enabled, lifecycle policy keeps only the last 5 images per repo.
- **S3 — artifacts bucket** (`artifacts.tf`) — private bucket (all public access blocked, no bucket policy) holding the Ansible convergence bundle CI uploads; versioned, with a 30-day expiration lifecycle rule on old/noncurrent versions.
- **S3 — frontend bucket** (`frontend-cdn.tf`) — private bucket (all public access blocked) holding the built SPA assets; readable only via CloudFront's Origin Access Control, enforced by a bucket policy scoped to that specific distribution's ARN.
- **CloudFront** (`frontend-cdn.tf`) — single distribution fronting both origins: the S3 frontend bucket (via OAC, for `/`, `/index.html`, `/assets/*` — cached) and the EC2 Elastic IP over HTTP (default behavior, everything else — API calls and short-code redirects — with caching disabled so click counts stay accurate). A random shared secret is injected as the `X-Origin-Verify` header so nginx on the instance can reject any request that didn't come through CloudFront.
- **RDS** (`rds.tf`) — one PostgreSQL 16 instance, `publicly_accessible = false`, deployed in the private subnets via a DB subnet group, encrypted storage, pinned to the same AZ as the EC2 instance to avoid cross-AZ transfer charges. Backups are enabled (1-day retention — free-tier accounts cap it below the usual 7) and a final snapshot (timestamped) is taken on destroy.
- **SSM Parameter Store** (`ssm.tf`) — namespaced under `/url-shortener/`, split into `config/*` (plain `String`, readable by the CI deploy role: instance id, public base URL, CloudFront distribution id, artifacts bucket name) and `secret/*` (`SecureString`, decryptable only by the EC2 instance role: database URL, the CloudFront origin-verify secret, and the Tailscale auth key, which is seeded manually and has drift ignored).
- **IAM / OIDC (CI identity)** (`oidc.tf`) — a GitHub Actions OIDC provider plus an IAM role GitHub assumes via `AssumeRoleWithWebIdentity`, restricted by the token's `sub` claim to this repo's `main` branch and pull requests. Two inline policies attach to it: one broad Terraform-management policy (`ec2:*`, `rds:*`, `cloudfront:*`, project-scoped `ecr:*`/`iam:*`/`ssm:*`, the Terraform state bucket, and the project's S3 buckets) used by `terraform.yml`, and one narrower deploy policy (S3 sync on the frontend/artifacts buckets, `cloudfront:CreateInvalidation`, `ssm:SendCommand`/`GetCommandInvocation`, and `ssm:PutParameter`/`GetParameter` scoped to `config/*` only — never `secret/*`) used by the future CD workflow.
